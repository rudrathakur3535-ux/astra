...
class PlannerEngine(PlannerPort):
    ...
    def create_plan(self, goal: Goal) -> Plan:
        ...
        # 1. Check PlanCache
        cached_steps_data = self.plan_cache.get_plan(goal.description)
        if cached_steps_data:
            logger.info(f"PlanCache hit for goal: '{goal.description}'")
            steps = [PlanStep.from_dict(d) for d in cached_steps_data]
            plan = Plan(goal=goal, steps=steps, status=PlanStatus.DRAFT)
            plan.add_log("Retrieved execution plan from PlanCache.")
            return plan

        # 2. Heuristic / Rule-based Planner for common Astra goals
        steps = self._generate_heuristic_steps(goal.description)

        plan = Plan(goal=goal, steps=steps, status=PlanStatus.DRAFT)
        plan.add_log(f"Generated {len(steps)} plan steps.")
        return plan
